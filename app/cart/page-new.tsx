...
import { ArrowLeft, Minus, Plus, Trash2, ShoppingBag, Gift, Truck, CreditCard, Loader2 } from "lucide-react"
import { cartApi, promotionsApi, ordersApi } from "@/lib/api"
...
export default function CartPage() {
  ...
  const applyPromoCode = async () => {
    ...
    try {
      ...
      const response = await promotionsApi.validatePromoCode(promoCode, subtotal)
      setAppliedPromo({
        code: promoCode,
  ...
  const subtotal = cartItems.reduce((sum, item) => sum + item.BasePrice * item.Quantity, 0)
  const savings = 0 // TODO: Calculate savings from original prices
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            disabled={updatingItem === item.CartID}
                          >
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() => updateQuantity(item.CartID, item.Quantity + 1)}
                            disabled={updatingItem === item.CartID || item.Quantity >= item.Stock}
                          >
